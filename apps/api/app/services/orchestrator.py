...
def _task_for_level(level: int, skills: list[str]) -> dict:
    return {"title": "تهيئة مشروع Web بسيط", "brief": "أنشئ هيكل المشروع وصفحة بداية صغيرة تعمل محليًا. ركّز في هذه المهمة على التهيئة فقط.", "criteria": ["هيكل ملفات واضح", "صفحة تعمل محليًا", "تعليمات تشغيل قصيرة", "رفع التعديل إلى GitHub"]}
...
def evaluate_task(db: Session, task: Task) -> list[Evaluation]:
    ...
    fallback = {"senior": {"outcome": "improve", "scores": {"تنفيذ المطلوب": 3, "جودة الكود": 3, "المنطق": 3, "الأمان والاختبارات": 3}, "analysis": "تعذر الحصول على تحليل موثوق من المودل، لذلك لن تُعتمد المحاولة تلقائيًا. ناقش Senior وتحقق من معايير النجاح.", "improvements": ["شغّل الكود وتحقق من النتيجة", "راجع كل معيار نجاح", "تحقق من حالات الخطأ"], "rationale": "يلزم تحقق إضافي.", "confidence": 1}}
    evidence = {"task": task.title, "criteria": task.acceptance_criteria, "commit_sha": submission.commit_sha if submission else None,
        "submission_summary": submission.summary if submission else "", "challenges": submission.challenges if submission else "", "submitted_content": artifact.content[:30000] if artifact else "", "submission_kind": artifact.kind if artifact else "unknown", "conversation": transcript}
    ...
    for agent in ("senior",):
        ...
        evaluation = Evaluation(task_id=task.id, agent=agent, scores=scores,
            rationale=analysis,
            evidence=[{"type": "decision", "outcome": outcome, "improvements": improvements}, {"type": "submission", "sha": submission.commit_sha if submission else None, "kind": artifact.kind if artifact else "unknown"}, {"type": "conversation", "message_ids": [message.id for message in messages[-10:]]}],
...
def weekly_report(db: Session, cycle: WorkCycle) -> WeeklyReport:
    ...
    report = {"kind": "weekly", "completed_tasks": len(completed), "senior_report": senior_report,
        ...
        "previous_performance_score": old_score, "performance_score": performance_score,
    ...
    if profile:
        ...
        assessment = dict(path.get("assessment", {})); assessment["starting_difficulty"] = report["next_difficulty"]
        project = dict(path.get("project", {})); project["status"] = "week_complete"
        path["assessment"] = assessment; path["performance_score"] = performance_score; path["last_weekly_report"] = report; path["project"] = project; profile.career_path = path
